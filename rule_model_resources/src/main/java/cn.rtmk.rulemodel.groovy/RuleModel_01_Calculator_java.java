 ...
 * 事件次数类条件的运算机逻辑
 */
public class RuleModel_01_Calculator_java implements RuleCalculator {
    private Jedis jedis;
     ...
     */
    @Override
    public void init(Jedis jedis, JSONObject ruleDefineParamJsonObject, RoaringBitmap profileUserBitmap, Collector<JSONObject> out) {
        this.jedis = jedis;
...
//                log.info("收到用户:{},行为事件:{}，处理规则:{}", userEvent.getGuid(), userEvent.getEventId(), ruleEntry.getKey());
    ...
    public void calc(UserEvent eventBean) throws ParseException {
        ...
        for (int i = 0; i < eventParams.size(); i++) {
            ...
            //先判断输入的用户行为事件，是否处于规则参数约定的计算时间窗口内
