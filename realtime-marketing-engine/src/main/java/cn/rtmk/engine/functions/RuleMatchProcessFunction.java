...
import cn.rtmk.engine.utils.UserEventComparator;
...
public class RuleMatchProcessFunction extends KeyedBroadcastProcessFunction<Integer, UserEvent, RuleMetaBean, RuleMatchResult> {
     ...
     */
    @Override
    public void processElement(UserEvent userEvent, KeyedBroadcastProcessFunction<Integer, UserEvent, RuleMetaBean, RuleMatchResult>.ReadOnlyContext readOnlyContext, Collector<RuleMatchResult> collector) throws Exception {
        ...
        for (Map.Entry<String, RuleMetaBean> ruleEntry : ruleEntries) {
            ...
            if (profileUserBitmap.contains(userEvent.getGuid())) {
                ...
                if (UserEventComparator.userEventIsEqualParam(userEvent, ruleTrigEvent)) {
                    ...
                    //如果已满足，则输出本规则的触达信息
                    if (isMatch) {
                        collector.collect(new RuleMatchResult(userEvent.getGuid(), ruleEntry.getKey(), System.currentTimeMillis()));
                    }
                }
     ...
     */
    @Override
    public void processBroadcastElement(RuleMetaBean ruleMetaBean, KeyedBroadcastProcessFunction<Integer, UserEvent, RuleMetaBean, RuleMatchResult>.Context context, Collector<RuleMatchResult> collector) throws Exception {
        ...
        try {
            ...
            if ("INSERT".equals(operateType) || "UPDATE".equals(operateType))  {
                ...
                RuleConditionCaculator ruleConditionCaculator = (RuleConditionCaculator) aClass.newInstance();
                //对规则运算器做初始化
                ruleConditionCaculator.init(jedis, JSON.parseObject(ruleMetaBean.getRuleParamJson()));
